...
describe("Inline literal operands in comparisons and enum nodes", () => {
  ...
  it("renders enum equality operands instead of the bare class name", () => {
    ...
    // Each enum-equality feeds the OR, so it renders as a parenthesized operand
    // — the nesting is explicit in the text rather than left to infix precedence.
    // (The enum labels carry their own "(ID n)" suffix, hence the loose tails.)
    expect(ascii).toMatch(/\(SearchState == Idle/);
    expect(ascii).toMatch(/OR \(SearchState == Patrolling/);
  });
...
describe("Explicit parentheses on nested math (node nesting, not infix precedence)", () => {
  // SET Total = (BaseDamage * Multiplier) + (Armor / Divisor). Two multiply/
  // divide nodes feed an Add node. UE's graph has no operator precedence — only
  // wires — so the export must parenthesize each nested operator to be
  // unambiguous from the text alone, rather than emitting "A * B + C / D".
  const NESTED_MATH = [
    'Begin Object Class=/Script/BlueprintGraph.K2Node_Event Name="Evt"',
    '   CustomProperties Pin (PinId=E1,PinName="then",Direction="EGPD_Output",PinType.PinCategory="exec",LinkedTo=(SetT ST_IN,))',
    'End Object',
    'Begin Object Class=/Script/BlueprintGraph.K2Node_VariableSet Name="SetT"',
    '   VariableReference=(MemberName="Total")',
    '   CustomProperties Pin (PinId=ST_IN,PinName="execute",PinType.PinCategory="exec",LinkedTo=(Evt E1,))',
    '   CustomProperties Pin (PinId=ST_V,PinName="Total",PinType.PinCategory="real",LinkedTo=(AddNode ADD_OUT,))',
    '   CustomProperties Pin (PinId=ST_OUT,PinName="then",Direction="EGPD_Output",PinType.PinCategory="exec")',
    'End Object',
    ...
    '   CustomProperties Pin (PinId=ADD_A,PinName="A",PinType.PinCategory="real",LinkedTo=(MulNode MUL_OUT,))',
    '   CustomProperties Pin (PinId=ADD_B,PinName="B",PinType.PinCategory="real",LinkedTo=(DivNode DIV_OUT,))',
    '   CustomProperties Pin (PinId=ADD_OUT,PinName="ReturnValue",Direction="EGPD_Output",PinType.PinCategory="real",LinkedTo=(SetT ST_V,))',
    'End Object',
    'Begin Object Class=/Script/BlueprintGraph.K2Node_PromotableOperator Name="MulNode"',
    '   FunctionReference=(MemberName="Multiply_DoubleDouble")',
    '   CustomProperties Pin (PinId=MUL_A,PinName="A",PinType.PinCategory="real",LinkedTo=(GetBase GB_OUT,))',
    '   CustomProperties Pin (PinId=MUL_B,PinName="B",PinType.PinCategory="real",LinkedTo=(GetMult GM_OUT,))',
    '   CustomProperties Pin (PinId=MUL_OUT,PinName="ReturnValue",Direction="EGPD_Output",PinType.PinCategory="real",LinkedTo=(AddNode ADD_A,))',
    'End Object',
    'Begin Object Class=/Script/BlueprintGraph.K2Node_PromotableOperator Name="DivNode"',
    '   FunctionReference=(MemberName="Divide_DoubleDouble")',
    '   CustomProperties Pin (PinId=DIV_A,PinName="A",PinType.PinCategory="real",LinkedTo=(GetArmor GA_OUT,))',
    '   CustomProperties Pin (PinId=DIV_B,PinName="B",PinType.PinCategory="real",DefaultValue="2.0")',
    '   CustomProperties Pin (PinId=DIV_OUT,PinName="ReturnValue",Direction="EGPD_Output",PinType.PinCategory="real",LinkedTo=(AddNode ADD_B,))',
    'End Object',
    'Begin Object Class=/Script/BlueprintGraph.K2Node_VariableGet Name="GetBase"',
    '   VariableReference=(MemberName="BaseDamage")',
    '   CustomProperties Pin (PinId=GB_OUT,PinName="BaseDamage",Direction="EGPD_Output",PinType.PinCategory="real",LinkedTo=(MulNode MUL_A,))',
    'End Object',
    'Begin Object Class=/Script/BlueprintGraph.K2Node_VariableGet Name="GetMult"',
    '   VariableReference=(MemberName="Multiplier")',
    '   CustomProperties Pin (PinId=GM_OUT,PinName="Multiplier",Direction="EGPD_Output",PinType.PinCategory="real",LinkedTo=(MulNode MUL_B,))',
    'End Object',
    'Begin Object Class=/Script/BlueprintGraph.K2Node_VariableGet Name="GetArmor"',
    '   VariableReference=(MemberName="Armor")',
    '   CustomProperties Pin (PinId=GA_OUT,PinName="Armor",Direction="EGPD_Output",PinType.PinCategory="real",LinkedTo=(DivNode DIV_A,))',
    'End Object',
  ].join("\n");

  it("wraps each nested operator in parentheses reflecting the node tree", () => {
    const r = parseBlueprint(NESTED_MATH);
    const ascii = renderASCII(r, { showDataPins: true });
    expect(ascii).toMatch(/\(BaseDamage \* Multiplier\) \+ \(Armor \/ 2\.0\)/);
  });

  it("does not wrap the top-level operator or its leaf (non-operator) operands", () => {
    const r = parseBlueprint(NESTED_MATH);
    const ascii = renderASCII(r, { showDataPins: true });
    // No redundant outer parens around the whole Add, and a bare variable/literal
    // operand is never parenthesized.
    expect(ascii).not.toMatch(/<- \(\(BaseDamage/);
    expect(ascii).not.toMatch(/\(BaseDamage\)/);
    expect(ascii).not.toMatch(/\(2\.0\)/);
  });
});

describe("Return node expressions are never truncated", () => {
  // A Return node whose ReturnValue is a long math chain. Everywhere else a wide
  // expression is clipped with "..." past the subline cap, but the Return node
  // carries the function's final result — clipping it mid-formula would make the
  // function impossible to verify from the text, so it renders in full.
  function buildLongReturn() {
    const lines = [
      'Begin Object Class=/Script/BlueprintGraph.K2Node_FunctionEntry Name="Entry"',
      '   FunctionReference=(MemberName="ComputeScore")',
      '   CustomProperties Pin (PinId=EN_OUT,PinName="then",Direction="EGPD_Output",PinType.PinCategory="exec",LinkedTo=(Ret RET_IN,))',
      'End Object',
      'Begin Object Class=/Script/BlueprintGraph.K2Node_FunctionResult Name="Ret"',
      '   CustomProperties Pin (PinId=RET_IN,PinName="execute",PinType.PinCategory="exec",LinkedTo=(Entry EN_OUT,))',
      '   CustomProperties Pin (PinId=RET_V,PinName="ReturnValue",PinType.PinCategory="real",LinkedTo=(Add0 OUT0,))',
      'End Object',
    ];
    // Chain of 6 Add nodes — shallow enough to stay within the depth-expansion
    // guard, but each adds a long distinctly-named variable so the fully-resolved
    // expression comfortably exceeds the 120-char subline cap.
    const N = 6;
    for (let i = 0; i < N; i++) {
      const next = i < N - 1 ? `(Add${i + 1} OUT${i + 1},)` : `(GetLeaf GL,)`;
      lines.push(
        `Begin Object Class=/Script/BlueprintGraph.K2Node_PromotableOperator Name="Add${i}"`,
        '   FunctionReference=(MemberName="Add_DoubleDouble")',
        `   CustomProperties Pin (PinId=A_A${i},PinName="A",PinType.PinCategory="real",LinkedTo=${next})`,
        `   CustomProperties Pin (PinId=A_B${i},PinName="B",PinType.PinCategory="real",LinkedTo=(GetRight${i} GR${i},))`,
        `   CustomProperties Pin (PinId=OUT${i},PinName="ReturnValue",Direction="EGPD_Output",PinType.PinCategory="real",LinkedTo=())`,
        'End Object',
        `Begin Object Class=/Script/BlueprintGraph.K2Node_VariableGet Name="GetRight${i}"`,
        `   VariableReference=(MemberName="RightOperandVariable${i}")`,
        `   CustomProperties Pin (PinId=GR${i},PinName="RightOperandVariable${i}",Direction="EGPD_Output",PinType.PinCategory="real",LinkedTo=(Add${i} A_B${i},))`,
        'End Object',
      );
    }
    lines.push(
      'Begin Object Class=/Script/BlueprintGraph.K2Node_VariableGet Name="GetLeaf"',
      '   VariableReference=(MemberName="LeafOperandVariable")',
      `   CustomProperties Pin (PinId=GL,PinName="LeafOperandVariable",Direction="EGPD_Output",PinType.PinCategory="real",LinkedTo=(Add${N - 1} A_A${N - 1},))`,
      'End Object',
    );
    return lines.join("\n");
  }

  it("renders the full Return expression even past the subline width cap", () => {
    const r = parseBlueprint(buildLongReturn());
    const ascii = renderASCII(r, { showDataPins: true });
    // The Return box's ReturnValue line is well over 120 chars and must not be
    // clipped: both the first and the deepest leaf operand are present, and no
    // ellipsis appears on it.
    const retLine = ascii.split("\n").find((l) => l.includes("ReturnValue:"));
    expect(retLine).toBeTruthy();
    expect(retLine.length).toBeGreaterThan(120);
    expect(retLine).toContain("RightOperandVariable0");
    expect(retLine).toContain("LeafOperandVariable");
    expect(retLine).not.toContain("...");
  });

  it("still clips an equally-long expression on a non-Return node", () => {
    // Same long chain, but consumed by a regular SET — the cap still applies
    // there, so the contrast confirms the exemption is Return-specific.
    const payload = buildLongReturn()
      .replace('Class=/Script/BlueprintGraph.K2Node_FunctionResult Name="Ret"', 'Class=/Script/BlueprintGraph.K2Node_VariableSet Name="Ret"')
      .replace('PinName="ReturnValue",PinType.PinCategory="real",LinkedTo=(Add0 OUT0,)', 'PinName="Score",PinType.PinCategory="real",LinkedTo=(Add0 OUT0,)');
    const r = parseBlueprint(payload);
    const ascii = renderASCII(r, { showDataPins: true });
    const setLine = ascii.split("\n").find((l) => /Score:.*\.\.\./.test(l));
...
describe("Enum display names resolve under qualified (UserDefinedEnum) serialization", () => {
  // The bug's exact shape: a UserDefinedEnum whose EnumEntries serialize fully
  // qualified ("E_WeatherState::NewEnumeratorN"), while the EnumEquality literal
  // and the SwitchEnum case pin names appear bare ("NewEnumeratorN"). The label
  // lookup must normalize the qualifier on both sides, otherwise every enumerator
  // falls back to its internal autogenerated name.
  function weatherGraph({ omitStormFriendly = false } = {}) {
    const friendly = (idx, key, display) =>
      omitStormFriendly && idx === 2
        ? null
        : `   EnumFriendlyNames(${idx})=NSLOCTEXT("", "${key}", "${display}")`;
    const lines = [
      'Begin Object Class=/Script/BlueprintGraph.K2Node_Event Name="Evt"',
      '   CustomProperties Pin (PinId=E1,PinName="then",Direction="EGPD_Output",PinType.PinCategory="exec",LinkedTo=(Branch BR_IN,))',
      'End Object',
      ...
      '   CustomProperties Pin (PinId=BR_IN,PinName="execute",PinType.PinCategory="exec",LinkedTo=(Evt E1,))',
      '   CustomProperties Pin (PinId=BR_C,PinName="Condition",PinType.PinCategory="bool",LinkedTo=(Eq EQ_OUT,))',
      '   CustomProperties Pin (PinId=BR_T,PinName="then",Direction="EGPD_Output",PinType.PinCategory="exec",LinkedTo=(Switch SW_IN,))',
      '   CustomProperties Pin (PinId=BR_E,PinName="else",Direction="EGPD_Output",PinType.PinCategory="exec")',
      'End Object',
      // EnumEquality: A is a Get of the enum var, B is the bare literal NewEnumerator2.
      'Begin Object Class=/Script/BlueprintGraph.K2Node_EnumEquality Name="Eq"',
      '   CustomProperties Pin (PinId=EQ_A,PinName="A",PinType.PinCategory="byte",PinType.PinSubCategoryObject=Enum\'"/Game/Weather/E_WeatherState.E_WeatherState"\',LinkedTo=(Get G_OUT,))',
      '   CustomProperties Pin (PinId=EQ_B,PinName="B",PinType.PinCategory="byte",PinType.PinSubCategoryObject=Enum\'"/Game/Weather/E_WeatherState.E_WeatherState"\',DefaultValue="NewEnumerator2")',
      '   CustomProperties Pin (PinId=EQ_OUT,PinName="bResult",Direction="EGPD_Output",PinType.PinCategory="bool",LinkedTo=(Branch BR_C,))',
      'End Object',
      'Begin Object Class=/Script/BlueprintGraph.K2Node_VariableGet Name="Get"',
      '   VariableReference=(MemberName="CurrentWeatherState")',
      '   CustomProperties Pin (PinId=G_OUT,PinName="CurrentWeatherState",Direction="EGPD_Output",PinType.PinCategory="byte",PinType.PinSubCategoryObject=Enum\'"/Game/Weather/E_WeatherState.E_WeatherState"\',LinkedTo=(Eq EQ_A,))',
      'End Object',
      'Begin Object Class=/Script/BlueprintGraph.K2Node_SwitchEnum Name="Switch"',
      '   Enum=Enum\'"/Game/Weather/E_WeatherState.E_WeatherState"\'',
      ...
      '   EnumEntries(3)="E_WeatherState::NewEnumerator3"',
      '   EnumEntries(4)="E_WeatherState::NewEnumerator4"',
      friendly(0, "E_WeatherState::NewEnumerator0", "Clear"),
      friendly(1, "E_WeatherState::NewEnumerator1", "Rain"),
      friendly(2, "E_WeatherState::NewEnumerator2", "Storm"),
      friendly(3, "E_WeatherState::NewEnumerator3", "DistantThunder"),
      friendly(4, "E_WeatherState::NewEnumerator4", "Windy"),
      '   CustomProperties Pin (PinId=SW_IN,PinName="Selection",Direction="EGPD_Input",PinType.PinCategory="exec",LinkedTo=(Branch BR_T,))',
      '   CustomProperties Pin (PinId=SW0,PinName="NewEnumerator0",Direction="EGPD_Output",PinType.PinCategory="exec")',
      '   CustomProperties Pin (PinId=SW1,PinName="NewEnumerator1",Direction="EGPD_Output",PinType.PinCategory="exec")',
      '   CustomProperties Pin (PinId=SW2,PinName="NewEnumerator2",Direction="EGPD_Output",PinType.PinCategory="exec")',
      '   CustomProperties Pin (PinId=SW3,PinName="NewEnumerator3",Direction="EGPD_Output",PinType.PinCategory="exec")',
      '   CustomProperties Pin (PinId=SW4,PinName="NewEnumerator4",Direction="EGPD_Output",PinType.PinCategory="exec")',
      'End Object',
    ];
    return lines.filter((l) => l !== null).join("\n");
  }

  it("renders an enum-equality literal as its DisplayName (E_WeatherState == Storm)", () => {
    const r = parseBlueprint(weatherGraph());
    const ascii = renderASCII(r, { showDataPins: true });
    expect(ascii).toMatch(/CurrentWeatherState == Storm/);
  });

  it("renders every SwitchEnum case pin as its DisplayName", () => {
    const r = parseBlueprint(weatherGraph());
    const ascii = renderASCII(r, { showDataPins: false });
    for (const label of ["Clear", "Rain", "Storm", "DistantThunder", "Windy"]) {
      expect(ascii).toMatch(new RegExp("├── " + label));
    }
  });

  it("never leaks the internal autogenerated name or its :: qualifier", () => {
    const r = parseBlueprint(weatherGraph());
    const ascii = renderASCII(r, { showDataPins: true });
    expect(ascii).not.toMatch(/NewEnumerator/);
    expect(ascii).not.toMatch(/E_WeatherState::/);
  });

  it("falls back to the bare internal name (never blank) when an entry has no DisplayName override", () => {
    // Index 2 (NewEnumerator2 / Storm) has no EnumFriendlyNames entry. Its
    // switch case and the equality literal must keep the readable bare internal
    // name rather than rendering blank or the qualified token.
    const r = parseBlueprint(weatherGraph({ omitStormFriendly: true }));
    const ascii = renderASCII(r, { showDataPins: true });
    expect(ascii).toMatch(/├── NewEnumerator2/);
    expect(ascii).toMatch(/CurrentWeatherState == NewEnumerator2/);
    expect(ascii).not.toMatch(/E_WeatherState::/);
    // The other entries still resolve to their display names.
    expect(ascii).toMatch(/├── Clear/);
