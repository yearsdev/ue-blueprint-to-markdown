...
function getDataInputs(node, byName, enumRegistry) {
  ...
  for (const p of node.pins) {
    ...
    if (p.LinkedTo.length > 0) {
      ...
      } else if (isSelectNode(sourceNode)) {
        // A Select is pure (no exec pins) and otherwise collapses to a bare
        // "Select on <Enum>", hiding both the selector and which value each enum
        // case yields. Surface the index source and the per-case options, with
        // option pins labeled by their enum DisplayName.
        const bindings = describeSelectNodeBindings(sourceNode, byName, enumRegistry);
        if (bindings.length > 0) entry.bindings = bindings;
      }
      out.push(entry);
...
function isSelectNode(node) {
  return node && node.nodeClass === "K2Node_Select";
}

// Render one value-carrying pin (the Select's index or one of its options) as a
// "label ← wired source" or "label = literal" binding line. Returns null when
// the pin carries neither a wire nor a meaningful literal.
function describeSelectPinValue(pin, label, byName, enumRegistry) {
  if (pin.LinkedTo.length > 0) {
    const link = pin.LinkedTo[0];
    const resolved = resolveThroughKnots(link.nodeName, link.pinName, byName);
    // depth=1 mirrors the Make-binding expansion: deep enough to name the source,
    // shallow enough not to recurse into an expansion with no render slot.
    const value = describeDataSource(byName.get(resolved.nodeName), resolved.pinName, byName, 1, enumRegistry);
    return label + " ← " + value;
  }
  const literal = pinLiteralValue(pin);
  if (literal === null || literal === "" || literal === "None") return null;
  const enumInfo = resolveEnumDefault(pin, enumRegistry);
  return label + " = " + (enumInfo ? formatEnumLabel(literal, enumInfo) : literal);
}

// A K2Node_Select picks one Option value by its Index pin. It's pure (no exec
// pins), so on its own it collapses to a bare "Select on <Enum>" with the
// selector and the case→value mapping invisible. Expand it into bindings: the
// Index source first, then one line per option pin. For an enum select the
// option pin names are the enumerators (NewEnumeratorN); resolve each to its
// DisplayName via the node's embedded table so cases read as Clear/Rain/Storm
// rather than the internal autogenerated names. Non-enum (int/bool) selects
// have no table, so options fall back to their pin friendly/name (Option 0, …).
function describeSelectNodeBindings(node, byName, enumRegistry) {
  if (!node._enumMap) node._enumMap = parseEnumMap(node.raw);
  const bindings = [];

  const indexPin = node.pins.find(
    (p) => p.PinName === "Index" && p.Direction === "EGPD_Input" && !isExecPin(p)
  );
  if (indexPin) {
    const line = describeSelectPinValue(indexPin, "Index", byName, enumRegistry);
    if (line) bindings.push(line);
  }

  for (const p of node.pins) {
    if (isExecPin(p)) continue;
    if (p.Direction !== "EGPD_Input") continue;
    if (p.PinName === "Index" || p.PinName === "self") continue;
    const info = node._enumMap.get(bareEnumeratorName(p.PinName));
    const label = info
      ? formatEnumLabel(p.PinName, info)
      : (p.PinFriendlyName || bareEnumeratorName(p.PinName));
    const line = describeSelectPinValue(p, label, byName, enumRegistry);
    if (line) bindings.push(line);
  }
  return bindings;
}
